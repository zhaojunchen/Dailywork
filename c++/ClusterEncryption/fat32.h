...
//短文件目录项32字节
typedef struct shortFDT {
    BYTE filename[8];//第一部分文件名
    BYTE extname[3];//文件扩展名
    BYTE attr;//属性 0F则说明是长文件需要索引到非0F，然后倒着读回来
    BYTE reserve;
    BYTE time1;
    BYTE creattime[2];
    BYTE createdate[2];
    BYTE visittime[2];
    BYTE high_cluster[2];//文件起始簇号高16位
    BYTE changetim2[2];
    BYTE changedate[2];
    BYTE low_cluster[2];//文件起始簇号低16位
    BYTE filelen[4];//文件长度
} SDT, *pSDT;

typedef struct longFDT {
